 ...
 * Date:    2019-04-25
 */
class RetryFunction(private val retry: RetryConfig) : Function<Observable<Throwable>, Observable<*>> {
    // 已经重试的次数
    private var count = 0
    ...
    override fun apply(observable: Observable<Throwable>): Observable<*> {
        return observable.flatMap {
            ...
            Log.d("RxRetrofit", "RetryFunc get error:$it,current count:$count, total retry.count:${retry.count}")
